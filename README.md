...
- 本项目使用定时编译（北京时间每月15日下午12点30分开始运行编译）、手动触发编译，以及推送触发编译（`main`、`test/**`、`dev/**` 分支更新 `README.md`、`script.sh`、`config.sh`、`files/**`、`.github/workflows/**` 后可开始编译）三种方式。
- 本项目编译固件适配斐讯 N1 盒子，用作旁路由。
- 默认 IP：`192.168.31.254`，主路由 IP：`192.168.31.1`，默认用户名/密码：`root/password`。
- 默认旁路由配置：N1 LAN 使用静态 IP，网关/DNS 指向主路由，LAN DHCP 默认关闭，不配置 WAN 口，并默认关闭 IPv6，避免和主路由 DHCP/IPv6 配置冲突。
- 发布 Release 需要在仓库 Secrets 中配置 `actions_release`，并确保该 Token 具备创建/更新 Release 与删除旧 Release 的权限；工作流仅在 `main` 分支、固件产物整理成功且 Token 已配置时发布和清理 Release，Token 仅在检测、发布和清理旧 Release 步骤中注入，测试分支只编译和上传 Artifact。
...
**更新日志**
- 20260709 修复工作流 `xfsprogs` 补丁顺序与命中验证、打包产物校验、Release 条件保护、保留最新 10 个 Release、权限/Token 使用范围、分支测试触发、`ksmbd-tools` 依赖警告、关闭 IPv6、第三方 Action 浮动引用与脚本路径/重复执行清理逻辑
